...
# Fenêtre (en bougies 1H clôturées) dans laquelle on cherche
# un contact avec l'AMA avant la bougie de reclaim actuelle.
# 1 = comportement historique (uniquement n-1 -> n).
TRIGGER_AMA_REBOUND_LOOKBACK = int(
    os.getenv("TRIGGER_AMA_REBOUND_LOOKBACK", "3")
)

# Fenêtre (en bougies 1H) utilisée uniquement pour le
# diagnostic : distance jusqu'au dernier contact AMA, même
# au-delà de TRIGGER_AMA_REBOUND_LOOKBACK, pour calibrer ce
# paramètre.
TRIGGER_AMA_DIAGNOSTIC_LOOKBACK = int(
    os.getenv("TRIGGER_AMA_DIAGNOSTIC_LOOKBACK", "20")
)
...
# ============================================================
# REBOND AMA — FENÊTRE ÉLARGIE
#
# Au lieu de comparer uniquement la bougie n-1 à la bougie n,
# on cherche le contact le plus récent avec l'AMA dans une
# fenêtre de lookback bougies, puis on vérifie que la bougie
# courante est la PREMIÈRE à clôturer de nouveau du bon côté
# de l'AMA depuis ce contact (pour éviter de re-déclencher à
# chaque bougie tant que le prix reste au-dessus/en-dessous).
# ============================================================
...
def find_ama_touch(
    ...
    """
    Cherche, en remontant depuis index-1, la bougie la plus
    récente où le prix était au contact ou du mauvais côté
    de l'AMA (un "touch"/pullback).

    Retourne l'indice de ce contact, ou None si aucun contact
    n'est trouvé dans la fenêtre.
    """

    start = index - 1
    stop = max(-1, index - lookback - 1)
...
def is_first_reclaim_since(
    ...
    """
    Vérifie qu'aucune bougie entre touch_index (exclu) et
    current_index (exclu) n'a déjà clôturé du bon côté de
    l'AMA — c'est-à-dire que la bougie courante est bien la
    première tentative de reclaim depuis ce contact.
    """

    for k in range(touch_index + 1, current_index):
...
def ama_rebound_signal(
    ...
    # Cas le plus courant : mèche + reclaim sur la MÊME
    # bougie (le low/high de la bougie courante touche déjà
    # l'AMA, et sa clôture repasse du bon côté). Pas besoin
    # de chercher plus loin dans ce cas.

    if direction == "LONG":

        if lows[index] <= ama[index]:
            return True

    else:

        if highs[index] >= ama[index]:
            return True

    # Sinon : cherche un contact sur une bougie précédente
    # dans la fenêtre, et vérifie que la bougie courante est
    # bien la première à reclaim depuis ce contact.

    touch_index = find_ama_touch(
        closes,
        lows,
        highs,
        ama,
...
def bars_since_ama_touch(
    ...
    """
    Diagnostic uniquement : distance (en bougies) jusqu'au
    dernier contact avec l'AMA, même au-delà de la fenêtre de
    trading, pour calibrer TRIGGER_AMA_REBOUND_LOOKBACK.
    """
...
def compute_mtf_signal(
    ...
    try:
        ...
        # ----------------------------------------------------
        # DIAGNOSTIC — distance jusqu'au dernier contact AMA
        #
        # Calculé indépendamment du déclencheur retenu, pour
        # pouvoir calibrer TRIGGER_AMA_REBOUND_LOOKBACK a
        # posteriori sur les candidats 1D/4H alignés qui
        # n'ont pas déclenché de signal 1H.
        # ----------------------------------------------------

        ama_bars_since_touch_long = (
            bars_since_ama_touch(
                trigger_close,
                trigger_low,
                trigger_high,
                trigger_ama,
                trigger_index,
                "LONG",
                TRIGGER_AMA_DIAGNOSTIC_LOOKBACK,
            )
        )

        ama_bars_since_touch_short = (
            bars_since_ama_touch(
                trigger_close,
                trigger_low,
                trigger_high,
        ...
        # ----------------------------------------------------
        # DIAGNOSTIC D'ALIGNEMENT 1D/4H → 1H
        # ----------------------------------------------------
        # Important : les compteurs globaux de flip/rebond 1H peuvent
        # provenir d'actifs dont le 1D/4H n'est pas aligné. On conserve
        # donc séparément le déclencheur brut et son déclencheur aligné.
        aligned_long_flip = (
            daily_bull
            and confirm_bull
            and trigger_flip_long
        )
        aligned_short_flip = (
            daily_bear
            and confirm_bear
            and trigger_flip_short
        )
        aligned_long_rebound = (
            daily_bull
            and confirm_bull
            and trigger_ama_rebound_long
        )
        aligned_short_rebound = (
            daily_bear
            and confirm_bear
            and trigger_ama_rebound_short
        )
        ...
        result.update({
            ...
            "alignedLongAmaRebound":
                aligned_long_rebound,

            "alignedShortAmaRebound":
                aligned_short_rebound,
            ...
            "amaBarsSinceTouchLong":
                ama_bars_since_touch_long,

            "amaBarsSinceTouchShort":
                ama_bars_since_touch_short,
...
def build_mtf_audit(
    ...
    return {
        ...
        # Déclencheurs 1H réellement alignés avec le sens 1D/4H.
        # Ces compteurs distinguent un déclencheur 1H existant
        # d'un déclencheur qui peut réellement produire un candidat MTF.
        "alignedLongFlip":
            sum(
                1
                for item in rows
        ...
        "alignedLongAmaRebound":
            sum(
                ...
                if (
                    item.get("dailyBull")
                    and item.get("confirmBull")
                    and item.get("triggerAmaReboundLong")
                )
            ),
        ...
        "alignedShortAmaRebound":
            sum(
                ...
                if (
                    item.get("dailyBear")
                    and item.get("confirmBear")
                    and item.get("triggerAmaReboundShort")
                )
            ),

        "alignedLongNoTrigger":
            sum(
                1
                for item in rows
                if (
                    item.get("dailyBull")
                    and item.get("confirmBull")
                    and not item.get("triggerLong")
                )
            ),

        "alignedShortNoTrigger":
            sum(
                1
                for item in rows
                if (
...
def build_html_report(
    ...
    # ----------------------------------------------------
    # DIAGNOSTIC 1H — candidats 1D/4H alignés sans
    # déclencheur 1H, avec distance au dernier contact AMA.
    # ----------------------------------------------------
    ...
    def diagnostic_sort_key(
        ...
        return (
            ...
            else TRIGGER_AMA_DIAGNOSTIC_LOOKBACK + 1,
            str(item.get("symbol", "")),
        )
    ...
    for item, direction in diagnostic_candidates:
        ...
        diagnostic_rows.append([
            ...
            fmt_num(
                item.get("triggerClose")
            ),
            (
                str(distance)
                if distance is not None
                else f"> {TRIGGER_AMA_DIAGNOSTIC_LOOKBACK}"
            ),
            (
                "Dans la fenêtre"
                if (
    ...
    if diagnostic_rows:
        ...
        diagnostic_html = render_table(
            diagnostic_rows,
            [
                "Symbol",
                "Direction",
                "Prix 1H",
                "Bougies depuis contact AMA",
                "Statut lookback",
            ],
        )
...
<h2>
9. Diagnostic 1H — candidats 1D/4H sans déclencheur
</h2>

<p class="small">

Candidats où le 1D et le 4H sont
alignés mais où aucun déclencheur 1H
(flip ou rebond AMA) n'est actif.

Fenêtre de trading actuelle :
{TRIGGER_AMA_REBOUND_LOOKBACK}
bougies 1H
(TRIGGER_AMA_REBOUND_LOOKBACK).

La colonne "Bougies depuis contact AMA"
indique la distance jusqu'au dernier
contact avec l'AMA, même au-delà de
cette fenêtre, pour aider à calibrer
le paramètre.
