...
# ---------------- GESTION DU RISQUE EMA50/EMA200 — INDÉPENDANTE ----------------
# Équivalent logique du Pine Script fourni : niveaux calculés à l'entrée
# sur le close 4H du croisement, puis suivi virtuel du SL/TP/Trailing.
...
EMA_RISK_STATE_FILE = os.getenv("EMA_RISK_STATE_FILE", "ema_risk_state.json")
...
def compute_ema_cross(asset: Dict[str, Any], warnings: List[str]) -> None:
    ...
    # Les bougies sont conservées temporairement pour la simulation du
    # portefeuille virtuel SL/TP/Trailing, sans nouvelle requête Binance.
    asset["_emaCrossClosedKlines"] = klines
...
# ============================================================
# GESTION DU RISQUE EMA50/EMA200 — SL / TP / TRAILING
# ============================================================
def load_ema_risk_state() -> Dict[str, Dict[str, Any]]:
    try:
        with open(EMA_RISK_STATE_FILE, "r", encoding="utf-8") as handle:
            data = json.load(handle)
        return data if isinstance(data, dict) else {}
    except FileNotFoundError:
        return {}
    except (OSError, json.JSONDecodeError) as exc:
        print(f"Avertissement gestion du risque EMA : état illisible ({exc}); état réinitialisé.")
        return {}


def save_ema_risk_state(state: Dict[str, Dict[str, Any]]) -> None:
    tmp_path = EMA_RISK_STATE_FILE + ".tmp"
    with open(tmp_path, "w", encoding="utf-8") as handle:
        json.dump(state, handle, ensure_ascii=False, indent=2, sort_keys=True)
    os.replace(tmp_path, EMA_RISK_STATE_FILE)
...
def _new_virtual_position(direction: str, entry_price: float, candle_close_ms: int, candle_close_utc: str) -> Dict[str, Any]:
    levels = risk_levels(entry_price, direction)
    return {
        "position": direction,
        "entryPrice": entry_price,
        "entryCandleCloseMs": candle_close_ms,
        "entryCandleCloseUtc": candle_close_utc,
        **levels,
        "trailingActive": False,
        "highestPrice": entry_price,
        "lowestPrice": entry_price,
        "trailingStopPrice": None,
        "lastProcessedCandleCloseMs": candle_close_ms,
        "lastEvent": None,
    }


def _exit_virtual_position(position: Dict[str, Any], reason: str, exit_price: float, candle: List[Any]) -> Dict[str, Any]:
    entry = as_float(position.get("entryPrice"))
    direction = position.get("position")
    pnl_pct = ((exit_price - entry) / entry * 100.0) if direction == "LONG" and entry else ((entry - exit_price) / entry * 100.0) if direction == "SHORT" and entry else 0.0
    close_ms = as_int(candle[6])
    close_utc = datetime.fromtimestamp(close_ms / 1000.0, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    return {
        ...
        "pnlPct": pnl_pct,
        "entryCandleCloseUtc": position.get("entryCandleCloseUtc"),
        "exitCandleCloseUtc": close_utc,
    }


def _process_risk_candle(symbol: str, position: Dict[str, Any], candle: List[Any]) -> Optional[Dict[str, Any]]:
    if len(candle) <= 6:
        return None
    ...
    close_ms = as_int(candle[6])
    direction = position.get("position")
    if direction not in {"LONG", "SHORT"} or high <= 0 or low <= 0:
        return None

    position["symbol"] = symbol

    # Même principe que le Pine fourni : SL/TP fixes à l'entrée et trailing
    # activé après +X% (long) ou -X% (short).
    if direction == "LONG":
        sl_hit = low <= as_float(position.get("slPrice"))
        tp_hit = high >= as_float(position.get("tpPrice"))
        if sl_hit:
            event = _exit_virtual_position(position, "SL Touché" if not tp_hit else "SL/TP touchés sur la même bougie — priorité SL conservatrice", as_float(position.get("slPrice")), candle)
            event["symbol"] = symbol
            return event
        if tp_hit:
            event = _exit_virtual_position(position, "TP Touché", as_float(position.get("tpPrice")), candle)
            event["symbol"] = symbol
            return event
        position["highestPrice"] = max(as_float(position.get("highestPrice"), high), high)
        if not position.get("trailingActive") and position["highestPrice"] >= as_float(position.get("trailActivationPrice")):
            position["trailingActive"] = True
        if position.get("trailingActive"):
            trail_stop = position["highestPrice"] * (1.0 - EMA_RISK_TRAIL_PERCENT / 100.0)
            previous_trail = position.get("trailingStopPrice")
            position["trailingStopPrice"] = max(as_float(previous_trail, 0.0), trail_stop)
            if low <= position["trailingStopPrice"]:
                event = _exit_virtual_position(position, "Trailing Stop Touché", position["trailingStopPrice"], candle)
                event["symbol"] = symbol
                return event
    else:
        sl_hit = high >= as_float(position.get("slPrice"))
        tp_hit = low <= as_float(position.get("tpPrice"))
        if sl_hit:
            event = _exit_virtual_position(position, "SL Touché" if not tp_hit else "SL/TP touchés sur la même bougie — priorité SL conservatrice", as_float(position.get("slPrice")), candle)
            event["symbol"] = symbol
            return event
        if tp_hit:
            ...
            event["symbol"] = symbol
            return event
        position["lowestPrice"] = min(as_float(position.get("lowestPrice"), low), low)
        if not position.get("trailingActive") and position["lowestPrice"] <= as_float(position.get("trailActivationPrice")):
            position["trailingActive"] = True
        if position.get("trailingActive"):
            trail_stop = position["lowestPrice"] * (1.0 + EMA_RISK_TRAIL_PERCENT / 100.0)
            previous_trail = position.get("trailingStopPrice")
            position["trailingStopPrice"] = min(as_float(previous_trail, float("inf")), trail_stop)
            if high >= position["trailingStopPrice"]:
                event = _exit_virtual_position(position, "Trailing Stop Touché", position["trailingStopPrice"], candle)
                event["symbol"] = symbol
                return event

    position["lastProcessedCandleCloseMs"] = close_ms
    return None


def merge_ema_risk_management(assets: List[Dict[str, Any]], ema_alerts: Dict[str, Any], warnings: List[str]) -> Dict[str, Any]:
    """Simule la gestion SL/TP/Trailing de la stratégie EMA50/200. Aucun ordre réel."""
    if not EMA_RISK_ENABLED:
        for asset in assets:
            asset.pop("_emaCrossClosedKlines", None)
        return {"events": [], "active": [], "enabled": False}

    state = load_ema_risk_state()
    events: List[Dict[str, Any]] = []
    new_alert_by_symbol = {a.get("symbol"): a for a in ema_alerts.get("alerts", [])}

    for asset in assets:
        symbol = asset.get("symbol", "-")
        klines = asset.pop("_emaCrossClosedKlines", [])
        if not klines:
            continue
        current_close_ms = as_int(asset.get("emaCrossCandleCloseMs"))
        current_close_utc = asset.get("emaCrossCandleCloseUtc", "-")
        position = state.get(symbol)

        # 1) Traiter les bougies 4H clôturées postérieures à la dernière bougie
        # traitée. La bougie du nouveau croisement n'est pas traitée comme une
        # bougie post-entrée : l'entrée est réputée se faire à son close.
        if isinstance(position, dict) and position.get("position") in {"LONG", "SHORT"}:
            last_processed = as_int(position.get("lastProcessedCandleCloseMs"), as_int(position.get("entryCandleCloseMs")))
            for candle in klines:
                close_ms = as_int(candle[6]) if len(candle) > 6 else 0
                if close_ms <= last_processed:
                    continue
                event = _process_risk_candle(symbol, position, candle)
                if event:
                    events.append(event)
                    position = None
        ...
        # 2) Un nouveau Golden/Death Cross ouvre ou inverse la position virtuelle
        # au close de la dernière bougie 4H, comme strategy.entry() dans le Pine.
        new_alert = new_alert_by_symbol.get(symbol)
        if new_alert:
            if position is not None:
                reversal_price = as_float(new_alert.get("price"))
                reversal_candle = klines[-1]
                reversal_event = _exit_virtual_position(position, "Croisement inverse — entrée opposée", reversal_price, reversal_candle)
                reversal_event["symbol"] = symbol
                events.append(reversal_event)
            direction = "LONG" if new_alert.get("direction") == "GOLDEN CROSS" else "SHORT"
            new_position = _new_virtual_position(direction, as_float(new_alert.get("price")), as_int(new_alert.get("candleCloseMs")), str(new_alert.get("candleCloseUtc", current_close_utc)))
            new_position["symbol"] = symbol
            state[symbol] = new_position
            position = new_position

        if position is not None:
            asset["riskPosition"] = position.get("position")
            asset["riskEntryPrice"] = position.get("entryPrice")
            asset["riskSLPrice"] = position.get("slPrice")
            asset["riskTPPrice"] = position.get("tpPrice")
            asset["riskTrailActivationPrice"] = position.get("trailActivationPrice")
            asset["riskTrailingActive"] = position.get("trailingActive")
            asset["riskTrailingStopPrice"] = position.get("trailingStopPrice")

    try:
        save_ema_risk_state(state)
    except OSError as exc:
        warnings.append(f"Gestion du risque EMA : impossible de sauvegarder l'état : {exc}")

    active = []
    for symbol, position in state.items():
        if not isinstance(position, dict) or position.get("position") not in {"LONG", "SHORT"}:
            continue
        active.append({"symbol": symbol, **position})
...
def ema_risk_html(ema_risk: Dict[str, Any]) -> str:
    if not ema_risk.get("enabled"):
        return "<hr><h2>12. Gestion du risque EMA50/EMA200</h2><p>Module désactivé.</p>"

    events = ema_risk.get("events", [])
    active = ema_risk.get("active", [])
    event_rows = []
    for e in events:
        event_rows.append(
            "<tr>"
            f"<td><b>{html_escape(e.get('symbol','-'))}</b></td>"
            f"<td>{html_escape(e.get('direction','-'))}</td>"
            f"<td>{html_escape(e.get('reason','-'))}</td>"
            f"<td>{format_number(e.get('entryPrice'), 8)}</td>"
            f"<td>{format_number(e.get('exitPrice'), 8)}</td>"
            f"<td>{format_number(e.get('pnlPct'), 2)}%</td>"
            f"<td>{html_escape(e.get('exitCandleCloseUtc','-'))}</td>"
            "</tr>"
        )
    active_rows = []
    for p in active:
        active_rows.append(
            "<tr>"
            f"<td><b>{html_escape(p.get('symbol','-'))}</b></td>"
            f"<td>{html_escape(p.get('position','-'))}</td>"
            f"<td>{format_number(p.get('entryPrice'), 8)}</td>"
            f"<td>{format_number(p.get('slPrice'), 8)}</td>"
            f"<td>{format_number(p.get('tpPrice'), 8)}</td>"
            f"<td>{format_number(p.get('trailActivationPrice'), 8)}</td>"
            f"<td>{'✅' if p.get('trailingActive') else '—'}</td>"
            f"<td>{format_number(p.get('trailingStopPrice'), 8)}</td>"
            "</tr>"
        )

    event_table = (
        "<table border='1' cellpadding='5' cellspacing='0'>"
        "<tr><th>Symbol</th><th>Sens</th><th>Sortie</th><th>Entrée</th><th>Sortie prix</th><th>P/L théorique</th><th>Clôture 4H</th></tr>"
        + "".join(event_rows) + "</table>"
        if event_rows else "<p>Aucune sortie SL/TP/Trailing ou inversion détectée sur ce scan.</p>"
    )
    active_table = (
        "<table border='1' cellpadding='5' cellspacing='0'>"
        "<tr><th>Symbol</th><th>Position</th><th>Entrée</th><th>SL</th><th>TP</th><th>Activation trailing</th><th>Trailing actif</th><th>Trailing Stop</th></tr>"
        + "".join(active_rows) + "</table>"
        if active_rows else "<p>Aucune position virtuelle EMA50/200 active.</p>"
    )
    return (
        "<hr><h2>12. Gestion du risque EMA50 / EMA200 — 4H</h2>"
        "<p>Simulation fidèle à la logique fournie : entrée au close du croisement, SL/TP fixes à l'entrée, "
        "puis trailing activé après la progression définie. <b>Aucun ordre réel n'est exécuté.</b></p>"
        f"<ul><li>Stop Loss : {ema_risk.get('slPercent', EMA_RISK_SL_PERCENT):.2f}%</li>"
        f"<li>Take Profit : {ema_risk.get('tpPercent', EMA_RISK_TP_PERCENT):.2f}%</li>"
        f"<li>Trailing : activation +/− {ema_risk.get('trailPercent', EMA_RISK_TRAIL_PERCENT):.2f}% et distance {ema_risk.get('trailPercent', EMA_RISK_TRAIL_PERCENT):.2f}%</li>"
        ...
        "<h3>Sorties / inversions détectées</h3>" + event_table +
        "<h3>Positions virtuelles actuellement ouvertes</h3>" + active_table +
        "<p><b>Note :</b> lorsqu'un même chandelier 4H touche simultanément SL et TP, l'outil retient le SL par prudence, "
        "car l'ordre intrabougie exact n'est pas observable avec les seules données OHLC.</p>"
    )
...
def build_report(assets: List[Dict[str, Any]], audit: CriterionAudit, warnings: List[str], errors: List[str], ema_diag: Optional[Dict[str, Any]] = None, ema_alerts: Optional[Dict[str, Any]] = None, ema_risk: Optional[Dict[str, Any]] = None) -> str:
    ...
    ema_risk = ema_risk or {"events": [], "active": [], "enabled": EMA_RISK_ENABLED, "slPercent": EMA_RISK_SL_PERCENT, "tpPercent": EMA_RISK_TP_PERCENT, "trailPercent": EMA_RISK_TRAIL_PERCENT}
    ...
    parts = [
        ...
        f"<li>Gestion du risque EMA : SL {EMA_RISK_SL_PERCENT:.2f}% | TP {EMA_RISK_TP_PERCENT:.2f}% | Trailing {EMA_RISK_TRAIL_PERCENT:.2f}%</li>",
...
def main() -> int:
    ...
    ema_risk: Dict[str, Any] = {"events": [], "active": [], "enabled": EMA_RISK_ENABLED, "slPercent": EMA_RISK_SL_PERCENT, "tpPercent": EMA_RISK_TP_PERCENT, "trailPercent": EMA_RISK_TRAIL_PERCENT}
    print("=" * 70)
    ...
    try:
        ...
        ema_risk = merge_ema_risk_management(assets, ema_alerts, warnings)
        print(
            "Diagnostic EMA200 : "
