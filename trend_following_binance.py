...
EMA_SURVEILLANCE_TOLERANCE_MINUTES = int(os.getenv("EMA_SURVEILLANCE_TOLERANCE_MINUTES", "25"))

EMA_RISK_ENABLED = os.getenv("EMA_RISK_ENABLED", "true").lower() == "true"
EMA_RISK_METHOD = os.getenv("EMA_RISK_METHOD", "atr").strip().lower()  # "atr" or "percent"
EMA_RISK_ATR_PERIOD = int(os.getenv("EMA_RISK_ATR_PERIOD", "14"))
EMA_RISK_ATR_MULTIPLIER = float(os.getenv("EMA_RISK_ATR_MULTIPLIER", "2.0"))
EMA_RISK_RR_RATIO = float(os.getenv("EMA_RISK_RR_RATIO", "2.0"))
...
def is_ema_surveillance_time(now: Optional[datetime] = None) -> bool:
    ...
    # The cron fires at :05, but GitHub Actions scheduled runs can start
    # several minutes late (more under load) — checking now.minute == 5
    # exactly means a slow start silently skips the 4H surveillance slot.
    # A tolerance window of a few minutes is safe: slots are 4h apart, so
    # there's no risk of bleeding into the next one.
    return now.hour in {0, 4, 8, 12, 16, 20} and 0 <= now.minute <= (5 + EMA_SURVEILLANCE_TOLERANCE_MINUTES)
...
def compute_ema_cross(asset: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        atr = atr_series(highs, lows, closes, EMA_RISK_ATR_PERIOD)
        i = len(closes) - 1
        p = i - 1
...
def risk_levels(entry: float, direction: str, atr: Optional[float] = None) -> Dict[str, float]:
    if entry <= 0:
        return {}

    use_atr = EMA_RISK_METHOD == "atr" and atr is not None and atr > 0
    if use_atr:
        distance = atr * EMA_RISK_ATR_MULTIPLIER
        reward = distance * EMA_RISK_RR_RATIO
        if direction == "LONG":
            return {
                "entry": entry,
                "sl": entry - distance,
                "tp": entry + reward,
                "trailActivation": entry + distance,
    ...
    # Fixed-percent fallback (used when EMA_RISK_METHOD="percent", or when
    # ATR could not be computed for this symbol on this run).
...
def build_ema_risk_parameters(alerts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for alert in alerts:
        ...
        row["risk"] = risk_levels(as_float(alert.get("price")), direction, alert.get("atr"))
        output.append(row)
    return output
...
def ema_risk_html(risk_rows: List[Dict[str, Any]]) -> str:
    ...
    for x in risk_rows:
        ...
        rows.append([
            ...
            fmt(r.get("entry")), fmt(r.get("atr")) if r.get("atr") is not None else "—",
            fmt(r.get("sl")), fmt(r.get("tp")),
            fmt(r.get("trailActivation")), fmt(r.get("trailDistance")),
        ])
    method_note = (
        f"<p><b>Méthode :</b> {'ATR adaptatif (x' + fmt(EMA_RISK_ATR_MULTIPLIER, 1) + ', période ' + str(EMA_RISK_ATR_PERIOD) + ', R:R x' + fmt(EMA_RISK_RR_RATIO, 1) + ')' if EMA_RISK_METHOD == 'atr' else 'Pourcentage fixe'} "
        "— le SL/TP/trailing s'élargit avec la volatilité du marché et se resserre quand elle est faible.</p>"
        if EMA_RISK_METHOD == "atr" else ""
    )
    note = (
        method_note +
    ...
    return note + render_table(
        ["Symbol", "Direction", "Entrée", "ATR", "SL", "TP",
