...
def merge_spot_order_book_depth(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        future_map = {executor.submit(fetch, a): a for a in assets}
        for future in concurrent.futures.as_completed(future_map):
            asset = future_map[future]
            try:
                _, book = future.result()
                mid = as_float(asset.get("lastPrice"))
                asset["depthTotalQuote"] = order_book_depth_metrics(book, mid)["depthTotalQuote"]
            except Exception as exc:
                asset["depthTotalQuote"] = 0.0
                warnings.append(f"Order book {asset.get('symbol')}: {exc}")
...
def screen_spot(
    ...
    assets = apply_criterion(
        ...
        lambda x: (
            "SPOT" in {str(p).upper() for p in (x.get("permissions") or [])}
            or "SPOT" in {str(p).upper() for p in (x.get("permissionSets") or [])}
        ),
    )
    assets = apply_criterion(
    ...
    if assets:
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            results = list(executor.map(check_history, assets))
...
def merge_trend_signals(assets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    workers = max(1, min(TREND_WORKERS, len(assets)))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        return list(executor.map(compute_trend_signal, assets))
...
def merge_ema_cross_alerts(
    ...
    # Intentionally no state file and no previous-alert comparison.
    workers = max(1, min(TREND_WORKERS, len(assets)))
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        results = list(executor.map(compute_ema_cross, assets))
    return [
        x for x in results
...
def audit_to_html(audit: CriterionAudit) -> str:
    ...
    for r in audit.rows:
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(r['criterion']))}</td>"
            f"<td>{r['before']}</td><td>{r['selected']}</td>"
            f"<td>{r['rejected']}</td><td>{r['retention']:.1f}%</td>"
            f"<td>{r['rejection']:.1f}%</td></tr>"
        )
    return (
        "<table><tr><th>Critère</th><th>Avant</th><th>Sélectionnés</th>"
        "<th>Rejetés</th><th>Rétention</th><th>Rejet</th></tr>"
        + "".join(rows) + "</table>"
    )
...
def signals_table_html(signals: List[Dict[str, Any]]) -> str:
    ...
    for x in valid[:EMAIL_TOP_RESULTS]:
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(x.get('symbol')))}</td>"
            f"<td>{html.escape(str(x.get('signalDirection')))}</td>"
            f"<td>{fmt(x.get('price'))}</td><td>{fmt(x.get('ema200'))}</td>"
            f"<td>{fmt(x.get('adx'), 2)}</td><td>{fmt(x.get('confidenceScore'), 1)}</td>"
            f"<td>{'✓' if x.get('macdOk') else '—'}</td>"
            f"<td>{'✓' if x.get('bollingerVolume') else '—'}</td>"
            f"<td>{'✓' if x.get('ichimoku') else '—'}</td>"
            f"<td>{html.escape(str(x.get('closeTimeUtc', '—')))}</td></tr>"
        )
    if not rows:
        return "<p>Aucun signal Trend Following sur la dernière bougie 1H clôturée.</p>"
    return (
        "<table><tr><th>Symbol</th><th>Direction</th><th>Prix</th><th>EMA200</th>"
        "<th>ADX</th><th>Score bonus</th><th>MACD</th><th>BB+Volume</th>"
        "<th>Ichimoku</th><th>Clôture 1H</th></tr>"
        + "".join(rows) + "</table>"
    )
...
def ema_cross_alerts_table_html(alerts: List[Dict[str, Any]]) -> str:
    ...
    for x in alerts:
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(x.get('symbol')))}</td>"
            f"<td>{html.escape(str(x.get('crossDirection')))}</td>"
            f"<td>{fmt(x.get('price'))}</td><td>{fmt(x.get('ema50'))}</td>"
            f"<td>{fmt(x.get('ema200'))}</td><td>{fmt(x.get('spreadPct'), 2)}%</td>"
            f"<td>{html.escape(str(x.get('closeTimeUtc')))}</td></tr>"
        )
    return (
        "<table><tr><th>Symbol</th><th>Alerte</th><th>Prix</th><th>EMA50</th>"
        "<th>EMA200</th><th>Écart</th><th>Clôture 4H</th></tr>"
        + "".join(rows) + "</table>"
    )
...
def ema_risk_html(risk_rows: List[Dict[str, Any]]) -> str:
    ...
    for x in risk_rows:
        ...
        rows.append(
            "<tr>"
            f"<td>{html.escape(str(x.get('symbol')))}</td>"
            f"<td>{html.escape(str(x.get('riskDirection')))}</td>"
            f"<td>{fmt(r.get('entry'))}</td><td>{fmt(r.get('sl'))}</td>"
            f"<td>{fmt(r.get('tp'))}</td><td>{fmt(r.get('trailActivation'))}</td>"
            f"<td>{fmt(r.get('trailDistance'))}</td></tr>"
        )
    return (
        ...
        "calcule aucun P/L.</p>"
        "<table><tr><th>Symbol</th><th>Direction</th><th>Entrée</th><th>SL</th>"
        "<th>TP</th><th>Activation trailing</th><th>Distance trailing</th></tr>"
        + "".join(rows) + "</table>"
    )
